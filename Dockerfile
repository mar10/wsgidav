...
#   docker run --rm -it -p <PORT>:8080 -v <ROOT_FOLDER>:/srv/wsgidav-share mar10/wsgidav:latest
...
#   docker run --rm -it -v <CONFIG_FILE>:/custom/wsgidav.yaml -p <PORT>:8080 -v <ROOT_FOLDER>:/srv/wsgidav-share mar10/wsgidav:latest
# Examples:
#   docker run --rm -it -p 8080:8080 -v c:/temp:/srv/wsgidav-share mar10/wsgidav:latest
#   docker run --rm -it -v ./wsgidav.yaml:/custom/wsgidav.yaml mar10/wsgidav:latest
# Then open (or enter this URL in Windows File Explorer or any other WebDAV client)
#   http://localhost:8080/
#
# Changes:
# - 2026-03-18: support for custom configuration file
...
RUN echo '#!/bin/sh' > /entrypoint.sh && \
    echo 'if [ -f "/custom/wsgidav.yaml" ]; then' >> /entrypoint.sh && \
    echo '  echo "Config file found, using configuration from /custom/wsgidav.yaml"' >> /entrypoint.sh && \
    echo '  exec wsgidav --host=0.0.0.0 --port=8080 --config=/custom/wsgidav.yaml' >> /entrypoint.sh && \
    echo 'else' >> /entrypoint.sh && \
    echo '  echo "No config file found, using default configuration"' >> /entrypoint.sh && \
    echo '  exec wsgidav --host=0.0.0.0 --port=8080 --root=/srv/wsgidav-share --auth=anonymous --no-config' >> /entrypoint.sh && \
