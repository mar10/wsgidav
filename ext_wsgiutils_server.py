...
def serve(conf, app):
     ...
     server = ExtServer((host, port), 
                              {'': app})
     if conf.get('verbose') > 0:
          print "PyFileServer serving at %s, port %s..." % (host, port)
...
config-file: 
  ...
  will look for a file named 'PyFileServer.conf' in the current directory"""
     ...
     from pyfileserver.mainappwrapper import PyFileApp
     app = PyFileApp(configfilespecified)
     optionsdict["verbose"] = app._verbose
          
     serve(optionsdict, app)
